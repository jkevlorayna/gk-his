...
class VillageRepository{
		function Get($id){
			global $conn;
			$query = $conn->query("SELECT * FROM tbl_village  WHERE Id = '$id'");
			return $query->fetch(PDO::FETCH_ASSOC);	
		}
		function Delete($id){
			global $conn;
			$query = $conn->prepare("DELETE FROM  tbl_village  WHERE Id = '$id'");
			$query->execute();	
		}
		function DataList($searchText,$pageNo,$pageSize){
			...
			$query = $conn->query("SELECT * FROM  tbl_village WHERE 1 = 1 $where $limitCondition");
			$count = $searchText != '' ?  $query->rowcount() : $conn->query("SELECT * FROM  tbl_village")->rowcount();
